...
export const AutocompleteDropdown = memo(
    forwardRef((props, ref) => {
        ...
        const valueFieldName = props.valueFieldName || 'title'
        ...
        useEffect(() => {
            if (selectedItem) {
                setSearchText(selectedItem[valueFieldName] ?? '')
            }
        ...
        const scrollContent = useMemo(() => {
            ...
            return (
                <FlatList
                    data={dataSet}
                    renderItem={(row, i) => {
                        const listItem = renderItem(row.item, searchText)
                        if (listItem) {
                            return (
                                <View>
                                    {listItem}
            ...
            >
                ...
                {isOpened && Array.isArray(dataSet) && (
                    ...
                    >
                        ...
                        {dataSet.length === 0 && !!searchText && <NothingFound />}
                    </View>
                )}
            </View>
        )
